class Solution:
    def solveSudoku(self, board: List[List[str]]) -> None:
        def backtrack(board, cq, rm, cm, bm, i, j):
            ...
            for k in range(9):
                if rm[i][k] == 0 and cm[j][k] == 0 and bm[boxIndex][k] == 0:
                    ...
                    board[i][j] = str(k+1)
                    if nextCell == [-1, -1] or backtrack(board, cq, rm, cm, bm, nextCell[0], nextCell[1]):
                        return True
        ...
        backtrack(board, cellQueue, rowMemo, colMemo, boxMemo, start[0], start[1])
